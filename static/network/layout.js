    ...
    .then(profile => {
        const profileForm = `
            <div class="popup-header">
                <h3>Edit Profile</h3>
                <button type="button" class="close-btn" onclick="remove_profile_popup()">×</button>
            </div>
            <form id="profile-edit-form" onsubmit="return updateProfile(event)">
                ...
                <div class="form-group">
                    ...
                    ${profile.profile_pic ? 
                        `<div id="preview-profile-pic" class="mt-2">
                            <img src="${profile.profile_pic}" class="preview-img">
                        </div>` : 
                        '<div id="preview-profile-pic" class="mt-2"></div>'
                    }
                </div>
                <div class="form-action-btns">
                    <input type="submit" class="btn submit-btn btn-success" value="Save Changes">
                </div>
            </form>`;
        
        document.querySelector('#profile-edit-container').innerHTML = profileForm;
        
        // Add preview functionality for profile picture
        document.querySelector('#profile_pic').onchange = previewProfilePic;
    })
    .catch(error => {
...
function updateProfile(event) {
    ...
    const profilePic = document.querySelector('#profile_pic').files[0];
    if (profilePic) {
        formData.append('profile_pic', profilePic);
    }

    fetch('/n/update_profile', {
        method: 'POST',
        headers: {
            'X-CSRFToken': getCookie('csrftoken')
        },
    ...
    .then(result => {
        if (result.success) {
            // Update profile info on page without reload
            document.querySelector('.profile-details h5').textContent = 
            ...
            if (result.profile_pic) {
                document.querySelector('.profile-image').style.backgroundImage = 
                    `url(${result.profile_pic})`;
                // Also update small profile pics if they exist
                document.querySelectorAll('.small-profilepic').forEach(pic => {
                    if (pic.closest('a').getAttribute('href') === `/${result.username}`) {
                        pic.style.backgroundImage = `url(${result.profile_pic})`;
                    }
                });
            }
            
            // Close popup
            remove_profile_popup();
...
function remove_profile_popup() {
    const popup = document.querySelector('#profile-popup');
    popup.style.display = 'none';
    document.querySelector('.body').setAttribute('aria-hidden', 'false');
    document.querySelector('body').style.overflow = "auto";
    document.querySelector('#profile-edit-container').innerHTML = '';
}

function previewProfilePic(event) {
    const file = event.target.files[0];
    const preview = document.querySelector('#preview-profile-pic');
